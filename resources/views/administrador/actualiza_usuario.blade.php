...
<div class="back-button">
    <a onclick="window.location.href='{{ route('admin.registro') }}'" class="btn btn-secondary">
...
<div class="container-">
    ...
    <form action="{{ route('admin.update_alumno', ['tipo', 'id_alumno' => $data_alumno->id_alumno]) }}" method="POST">
        ...
        <input type="text" name="matricula_alumno" value="{{ $data_alumno->matricula_alumno }}" placeholder="Matrícula del alumno" disabled>
        <br>
        ...
        <input type="text" name="apellido_paterno_alumno" value="{{ $data_alumno->apellido_paterno_alumno }}" placeholder="Apellido paterno del alumno" disabled>
        <br>
    ...
    <form action="{{ route('admin.update_docente', ['tipo', 'id_docente' => $data_docente->id_docente]) }}" method="POST">
        ...
        <input type="text" name="rfc_docente" value="{{ $data_docente->rfc_docente }}" placeholder="Clave del docente" disabled>
        <br>
        <label for="nombre_docente">Nombre del docente:</label>
        <input type="text" name="nombre_docente" value="{{ $data_docente->nombre_docente }}" placeholder="Nombre del docente" disabled>
        <br>
        <label for="apellido_paterno_docente">Apellido paterno:</label>
        <input type="text" name="apellido_paterno_docente" value="{{ $data_docente->apellido_paterno_docente }}" placeholder="Apellidos del docente" disabled>
        <br>
        <label for="apellido_materno_docente">Apellido paterno:</label>
        <input type="text" name="apellido_materno_docente" value="{{ $data_docente->apellido_materno_docente }}" placeholder="Apellidos del docente" disabled>
        <br>
    ...
    <div class="documentos">
        ...
        <div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                @if(Str::startsWith($archivo->tipo_archivo, 'image/'))
                <!-- Vista previa de imagen -->
                <img src="{{ asset('storage/' . $archivo->ruta_archivo) }}" alt="Vista previa" width="200">
                <a href="{{ route('admin.descargar-archivo', $archivo->id_archivo) }}" target="_blank">Descargar</a>
                @elseif($archivo->tipo === 'application/pdf')
                <!-- Vista previa PDF -->
                <iframe src="{{ asset('storage/' . $archivo->ruta_archivo) }}" width="auto" height="auto"></iframe>
                <a href="{{ route('admin.descargar-archivo', $archivo->id_archivo) }}" target="_blank">Descargar</a>
                @else

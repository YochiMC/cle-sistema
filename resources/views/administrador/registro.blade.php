...
<div class="container-">
    ...
    <div class="opciones">
        @can('crear usuarios')
        <button id="btn-abrir-modal" class="button"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-person-plus-fill" viewBox="0 0 16 16">
        ...
        <form method="GET" action="{{ route('admin.registro') }}">
            ...
            <div class="search-bar">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar por nombre, apellido o matrícula" autocomplete="off">
    ...
    <dialog id="modal" class="modal">
        ...
        <form action="{{ route('admin.registrar-usuario') }}" method="POST" class="form-agregar">
            ...
            <div id="datos_generales" style="display:none" class="contenedor-info-general">
                ...
                <input type="number" id="phonenumber" name="phonenumber" value="{{ old('phonenumber') }}" placeholder="Escribe el número de teléfono">
                @error('phonenumber')
                ...
                <input type="text" id="nombre" name="nombre" value="{{ old('nombre') }}" placeholder="Escribe el nombre(s)">
                @error('nombre')
                <span class="error">{{ $message }}</span>
                @enderror
                <label for="apellido_paterno">Apellido paterno:</label>
                <input type="text" id="apellido_paterno" name="apellido_paterno" value="{{ old('apellido_paterno') }}" placeholder="Escribe el apellido paterno">
                @error('apellido_paterno')
                <span class="error">{{ $message }}</span>
                @enderror
                <label for="apellido_materno">Apellido materno:</label>
                <input type="text" id="apellido_materno" name="apellido_materno" value="{{ old('apellido_materno') }}" placeholder="Escribe el apellido materno">
            ...
            <div id="maestroForm" style="display:none" class="contenedor-info-docente">
                ...
                <input type="text" id="rfc_docente" name="rfc_docente" value="{{ old('rfc_docente') }}" placeholder="Escribe el número de trabajador">
                @error('rfc_docente')
                <span class="error">{{ $message }}</span>
                @enderror
            </div>
            ...
            <tr>
                ...
                <td>
                    <div class="gestionar">
                        ...
                        <form action="{{ route('admin.usuarios.delete', $alumno->id_usuario) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
            ...
            <tr>
                ...
                <td class="infor">
                    <div class="gestionar">
                        ...
                        <form action="{{ route('admin.usuarios.delete', $docente->id_usuario) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro?')"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eraser" viewBox="0 0 16 16">
